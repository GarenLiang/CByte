...
if num is 24 then you can multiply 8 by 3 which produces 24, so your program should return 2 
because there is a total of only 2 digits that are needed. Another example: if num is 90, you can multiply 10 * 9, 
...
function ProductDigits(num) { 
  ...
  for(var i=0;i<=num;i++){
      if(num%i===0){
          factors.push([i.toString(),(num/i).toString()]);
      }
  }
  factors=factors.map(arr=>(arr[0]+arr[1]).length);
